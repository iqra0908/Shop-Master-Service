...
if (!function_exists('en_distancebase_ltl_update_now')) {
    ...
    function en_distancebase_ltl_update_now()
    {
        $index = 'distance-based-shipping-calculator/distance-base-shipping-calculator.php';
        $plugin_info = get_plugins();
        $plugin_version = (isset($plugin_info[$index]['Version'])) ? $plugin_info[$index]['Version'] : '';
        $update_now = get_option('en_distancebase_ltl_update_now');
        if ($update_now != $plugin_version) {
            EN_DISTANCE_BASE_SHIPPING_installation();
            update_option('en_distancebase_ltl_update_now', $plugin_version);
        }
    }
    add_action('init', 'en_distancebase_ltl_update_now');
}
/**
 * App uninstall hook
 */
...
function EN_DISTANCE_BASE_SHIPPING_admin_enqueue_scripts()
{
    if (isset($_REQUEST['tab']) && $_REQUEST['tab'] == 'distance_base_shipping') {
        ...
        wp_enqueue_script('EnDistanceBaseShippingAdminJs', EN_DISTANCE_BASE_SHIPPING_DIR_FILE . 'admin/assets/en-distance-base-shipping-admin.js', [], '2.0.1');
        wp_localize_script('EnDistanceBaseShippingAdminJs', 'script', [
        ...
        wp_enqueue_script('EnDistanceBaseShippingAdminPopupJs', EN_DISTANCE_BASE_SHIPPING_DIR_FILE . 'admin/popup/assets/js/en-distance-base-shipping-admin-popup.js', [], '2.0.1');
        wp_localize_script('EnDistanceBaseShippingAdminPopupJs', 'script', [
            'pluginsUrl' => EN_DISTANCE_BASE_SHIPPING_PLUGIN_URL,
        ]);

        wp_register_style('EnDistanceBaseShippingAdminCss', EN_DISTANCE_BASE_SHIPPING_DIR_FILE . 'admin/assets/en-distance-base-shipping-admin.css', false, '1.0.1');
        wp_enqueue_style('EnDistanceBaseShippingAdminCss');

        wp_register_style('EnDistanceBaseShippingAdminPopupCss', EN_DISTANCE_BASE_SHIPPING_DIR_FILE . 'admin/popup/assets/css/en-distance-base-shipping-admin-popup.css', false, '2.0.1');
...
{
    wp_enqueue_script('EnDistanceBaseShippingFrontEnd', EN_DISTANCE_BASE_SHIPPING_DIR_FILE . '/admin/assets/en-distance-base-shipping-admin-frontend.js', ['jquery'], '1.0.1');
