...
class EnDistanceBaseShippingPopupAjax
{
    ...
    public function suspend_automatic_detection_dbsc()
    {
        ...
        (isset($en_connection_settings_suspend_distance_base_shipping) && (!empty($en_connection_settings_suspend_distance_base_shipping))) ?
    ...
    public function en_add_shipping_zone()
    {
        ...
        if (!empty($form_data)) {
            global $wpdb;
            $append = '';
            $action = (isset($form_data['en_zone_action'])) ? $form_data['en_zone_action'] : '';
            $origin_id = (isset($form_data['origin_id'])) ? $form_data['origin_id'] : '';
            $profile_id = (isset($form_data['profile_id'])) ? $form_data['profile_id'] : '';
            $origin_order = (isset($form_data['origin_order'])) ? $form_data['origin_order'] : '';
            $zone_id = wc_clean(wp_unslash($form_data['zone_id']));
            ...
            $zone_joins = "SELECT oz.zone_id as zone_id FROM $en_origin_zones oz WHERE oz.en_origin_id  = $origin_id";
            $zone_ids = $wpdb->get_results($zone_joins);
            if (!empty($zone_ids)) {
                $valid_region = TRUE;
            ...
            if ($action == 'add_zone') {
                ...
                $en_shipping_origins = $wpdb->prefix . 'en_shipping_origins';
                $origins_query = "SELECT id FROM $en_shipping_origins WHERE profile_id = $profile_id AND origin_order = $origin_order";
                $en_shipping_origin_zones = $wpdb->get_results($origins_query);
    ...
    public function en_update_shipping_profile()
    {
        ...
        if (isset($_POST['form_data']) && !empty($_POST['form_data'])) {
            ...
            $en_profile_id = (isset($form_data['en_profile_id']) && !empty($form_data['en_profile_id'])) ? sanitize_text_field($form_data['en_profile_id']) : '';
            ...
            $profiles_query = "select * from $en_table where profile_nickname = '" . $profile_name . "'";
            $en_profile_exist = $wpdb->get_row($profiles_query);
            ...
            if (empty($en_selected_classes) && strlen($class_slug) > 0) {
            ...
            if ($en_profile_action == 'add_profile') {
                ...
                $en_add_new_profile_html = $this->en_add_new_profile_template($shipping_classes_arr, $en_profile_id, $profile_name, $profile_classes_data);
            } else {
                ...
                $en_classes_html = $this->en_classes_html($shipping_classes_arr);
            }
            echo wp_json_encode([
                'response' => 'success',
    ...
    public function en_edit_shipping_profile()
    {
        ...
        $profile_id = (isset($_POST['profile_id']) && !empty($_POST['profile_id'])) ? sanitize_text_field($_POST['profile_id']) : '';

        $en_table = $wpdb->prefix . 'en_profiles';
        $profiles_query = "select * from $en_table where id = $profile_id";
        $en_profile = $wpdb->get_row($profiles_query);
        ...
        if (isset($en_profile) && !empty($en_profile)) {
            $en_classes_details = (isset($en_profiles_classes_arr) && !empty($en_profiles_classes_arr)) ?
                $enDBSCRatesTemplateObj->enGetSelectedClassesDetails($en_profiles_classes_arr) : [];
     ...
     */
    public function en_add_new_profile_template($en_selected_classes, $en_profile_id, $profile_name, $profile_classes_data)
    {
        ...
        $enDBSCRatesTemplateObj = new EnDistanceBaseShippingRatesTemplateSettings();
        $en_classes_details = (isset($en_selected_classes) && !empty($en_selected_classes)) ? $enDBSCRatesTemplateObj->enGetSelectedClassesDetails($en_selected_classes) : [];
        ...
        $en_classes_details = (isset($en_selected_classes) && !empty($en_selected_classes)) ?
            $enDBSCRatesTemplateObj->enGetSelectedClassesDetails($en_selected_classes) : [];
     ...
     * @return string
     */
    public function en_classes_html($en_selected_classes)
    {
        $html = '';
        $enDBSCRatesTemplateObj = new EnDistanceBaseShippingRatesTemplateSettings();
        $en_classes_details = (isset($en_selected_classes) && !empty($en_selected_classes)) ?
    ...
    public function en_add_shipping_origin()
    {
        ...
        if (isset($_POST['form_data']) && !empty($_POST['form_data'])) {
            ...
            $en_shipping_origin_id = (isset($form_data['en_shipping_origin_id']) && !empty($form_data['en_shipping_origin_id'])) ? sanitize_text_field($form_data['en_shipping_origin_id']) : '';

            $profile_id = (isset($form_data['en_origin_profile_id']) && !empty($form_data['en_origin_profile_id'])) ? sanitize_text_field($form_data['en_origin_profile_id']) : '';

            // New change
            $en_add_the_shipping_origin_id = (isset($form_data['en_add_the_shipping_origin_id']) && !empty($form_data['en_add_the_shipping_origin_id'])) ? sanitize_text_field($form_data['en_add_the_shipping_origin_id']) : '';
            ...
            if ($en_add_the_shipping_origin_id > 0 && $en_add_the_shipping_origin == 'en_as_a_new_shipping_from_profile') {
                $origins_order_query = "SELECT origin_order FROM $en_table WHERE profile_id =  $profile_id ORDER BY origin_order DESC LIMIT 1";
                $max_origins_order = $wpdb->get_results($origins_order_query);
        ...
        if (!$en_shipping_suggestion_flag == 'yes') {
            // Validate address
            ...
            if ( $country_code == 'US') {
                ...
                foreach ($en_suggested_address_array as $key => $value) {
                    strlen($complete_Address) > 0 ? $complete_Address .= " ,$value" : $complete_Address = $value;
        ...
        if ($en_shipping_origin_action == 'add_shipping_origin') {
            ...
            if ($en_add_the_shipping_origin == 'en_to_this_shipping_from_profile') {
                ...
                $origins_order_query_find_zone = "SELECT * FROM $en_table o INNER JOIN $o_z_table oz ON o.id = oz.en_origin_id WHERE o.profile_id =  $profile_id AND o.origin_order = $en_origin_order";
                $finded_zone = $wpdb->get_results($origins_order_query_find_zone);
                $added_zone = [];
        ...
        $origins_order_query = "SELECT * FROM $en_table WHERE profile_id =  $profile_id AND origin_order =  $en_origin_order";
    ...
    function en_edit_shipping_origin()
    {
        ...
        $profile_id = (isset($_POST['profile_id']) && !empty($_POST['profile_id'])) ? sanitize_text_field($_POST['profile_id']) : '';
        $origin_id = (isset($_POST['origin_id']) && !empty($_POST['origin_id'])) ? sanitize_text_field($_POST['origin_id']) : '';

        $en_table = $wpdb->prefix . 'en_shipping_origins';
        $en_shipping_origin_query = "select * from `" . $en_table . "` where `id` = " . $origin_id . " AND `profile_id` = '" . $profile_id . "' ";
        $en_shipping_origin = $wpdb->get_row($en_shipping_origin_query);
    ...
    function en_add_zone_rate()
    {
        ...
        if (isset($_POST['form_data']) && !empty($_POST['form_data'])) {
            ...
            $min_weight = (isset($form_data['en_dbsc_add_rate_min_weight']) && !empty($form_data['en_dbsc_add_rate_min_weight'])) ? $form_data['en_dbsc_add_rate_min_weight'] : '0';

            $max_weight = (isset($form_data['en_dbsc_add_rate_max_weight']) && !empty($form_data['en_dbsc_add_rate_max_weight'])) ? $form_data['en_dbsc_add_rate_max_weight'] : 'up';
            ...
            $min_distance = (isset($form_data['en_dbsc_add_rate_min_distance']) && !empty($form_data['en_dbsc_add_rate_min_distance'])) ? $form_data['en_dbsc_add_rate_min_distance'] : '0';

            $max_distance = (isset($form_data['en_dbsc_add_rate_max_distance']) && !empty($form_data['en_dbsc_add_rate_max_distance'])) ? $form_data['en_dbsc_add_rate_max_distance'] : 'up';

            $min_length = (isset($form_data['en_dbsc_add_rate_min_length']) && !empty($form_data['en_dbsc_add_rate_min_length'])) ? $form_data['en_dbsc_add_rate_min_length'] : '0';

            $max_length = (isset($form_data['en_dbsc_add_rate_max_length']) && !empty($form_data['en_dbsc_add_rate_max_length'])) ? $form_data['en_dbsc_add_rate_max_length'] : 'up';
            ...
            $min_quote = (isset($form_data['en_dbsc_add_rate_min_quote']) && !empty($form_data['en_dbsc_add_rate_min_quote'])) ? $form_data['en_dbsc_add_rate_min_quote'] : '0';

            $max_quote = (isset($form_data['en_dbsc_add_rate_max_quote']) && !empty($form_data['en_dbsc_add_rate_max_quote'])) ? $form_data['en_dbsc_add_rate_max_quote'] : 'up';
            
            $min_cart_value = (isset($form_data['en_dbsc_add_rate_min_cart_value']) && !empty($form_data['en_dbsc_add_rate_min_cart_value'])) ? $form_data['en_dbsc_add_rate_min_cart_value'] : '';

            $max_cart_value = (isset($form_data['en_dbsc_add_rate_max_cart_value']) && !empty($form_data['en_dbsc_add_rate_max_cart_value'])) ? $form_data['en_dbsc_add_rate_max_cart_value'] : '';
            ...
            $en_profile_id = (isset($form_data['en_profile_id']) && !empty($form_data['en_profile_id'])) ? $form_data['en_profile_id'] : '';
            $zone_id = (isset($form_data['en_zone_id']) && !empty($form_data['en_zone_id'])) ? $form_data['en_zone_id'] : '';
            $rate_id = (isset($form_data['en_rate_id']) && !empty($form_data['en_rate_id'])) ? $form_data['en_rate_id'] : '';
            ...
            if ($inserted || $updated) {
                ...
                $min_quote = number_format($min_quote, 2, ".", "");
                $max_quote = $max_quote != 'up' ? number_format($max_quote, 2, ".", "") : 'up';
                $measured_by = $measured_by == 'route' ? 'Route' : 'Straight Line';
                $tr_id = "en_zone" . $zone_id . "_rate$rate_id";

                $max_distance_show = ($max_distance == 'up') ? $max_distance : $max_distance . ' ' . $unit;
                $max_weight_show = ($max_weight == 'up') ? $max_weight : $max_weight . ' ' . $wt_unit;
                $max_length_show = ($max_length == 'up') ? $max_length : $max_length . ' ' . $dim_unit;
                
                $calculate_for_label = ($calculate_for == 'item_after_quotes') ? 'Item' : ucfirst($calculate_for);
                $calculate_for_show = ($calculate_for == 'flat') ? '$' . $rate : '$' . $rate . ' / ' . $calculate_for_label;
    ...
    public
    function en_get_zone_rate()
    {
        global $wpdb;
        $result = [];

        $rate_id = isset($_POST['rate_id']) && !empty($_POST['rate_id']) ? $_POST['rate_id'] : 0;

        if ($rate_id) {
            $rate_table = $wpdb->prefix . 'en_dbsc_rates';
            $query = "SELECT * FROM $rate_table WHERE id=$rate_id";
            $rate = $wpdb->get_row($query);
    ...
    function en_dbsc_delete_record_action()
    {
        ...
        if (isset($_POST['form_data']) && !empty($_POST['form_data'])) {
            ...
            $profile_id = (isset($form_data['en_profile_id']) && !empty($form_data['en_profile_id'])) ? sanitize_text_field($form_data['en_profile_id']) : '';

            $origin_id = (isset($form_data['en_origin_id']) && !empty($form_data['en_origin_id'])) ? sanitize_text_field($form_data['en_origin_id']) : '';

            $zone_id = (isset($form_data['en_zone_id']) && !empty($form_data['en_zone_id'])) ? sanitize_text_field($form_data['en_zone_id']) : '';

            $rate_id = (isset($form_data['en_rate_id']) && !empty($form_data['en_rate_id'])) ? sanitize_text_field($form_data['en_rate_id']) : '';
            switch ($action) {
    ...
    public
    function delete_origin($origin_id, $profile_id = 0)
    {
        global $wpdb;
        $origin_table = $wpdb->prefix . 'en_shipping_origins';
        if ($profile_id) {
            $origin_ids = $wpdb->get_results("SELECT id FROM $origin_table WHERE profile_id=$profile_id");
    ...
    public
    function delete_zone($zone_id, $origin_id = 0)
    {
        global $wpdb;
        $origin_zone_table = $wpdb->prefix . 'en_origin_zones';
        if (!$zone_id > 0) {

            $zone_ids = $wpdb->get_results("SELECT zone_id FROM $origin_zone_table WHERE en_origin_id=$origin_id");
            foreach ($zone_ids as $zone_id) {
                // New Change
                $zones_data = WC_Shipping_Zones::get_zone(absint($zone_id->zone_id));
                if ($zones_data) {

                    $compare_zone_id = $wpdb->get_results("SELECT zone_id FROM $origin_zone_table WHERE zone_id=$zone_id->zone_id");
    ...
    public
    function delete_rate($rate_id, $zone_id = 0)
    {
        ...
        if (!$rate_id) {
            $rate_ids = $wpdb->get_col("SELECT dbsc_rate_id FROM $zone_rate_table WHERE zone_id=$zone_id");
