...
namespace DistanceBaseShippingOrderExport;

/**
 * Get and save settings.
 * Class DistanceBaseShippingOrderExport
 * @package DistanceBaseShippingOrderExport
 */
if (!class_exists('DistanceBaseShippingOrderExport')) {
    ...
    class DistanceBaseShippingOrderExport
    {
        ...
        public function en_order_details_by_id($order, $order_id, $current_date = '')
        {
            ...
            if (!empty($en_widget_details_data)) {
                foreach ($en_widget_details_data as $key => $shipments) {
                    $shipment_type = (is_array($shipments) && count($shipments) > 1) ? 'multiple' : 'single';
                    if (is_array($shipments) && !empty($shipments)) {
                        foreach ($shipments as $ship_nbr => $shipment) {
                            $plugin_name = 'distanceBasedShippingCalculator';
                            $cost = (isset($shipment['cost'])) ? $shipment['cost'] : 0;
                            $label = (isset($shipment['label'])) ? $shipment['label'] : '';
                            $en_orders[] = [
                                'orderId' => $order_id,
                                'serviceId' => $ship_nbr,
                                'serviceName' => $label,
        ...
        public function en_orders_sending($orders, $en_orders_ids)
        {
            $post_data = [
                ...
                'licenseKey' => get_option('en_connection_settings_license_key_distance_base_shipping'),
                'currencyUnit' => get_option('woocommerce_currency'),
                'platform' => 'wordpress',
                'orders' => $orders,
            ];

            $response = \EnDistanceBaseShippingCurl\EnDistanceBaseShippingCurl::EN_DISTANCE_BASE_SHIPPING_sent_http_request(EN_DISTANCE_BASE_SHIPPING_ORDER_EXPORT_HITTING_URL, $post_data, 'POST', 'Export Order');
