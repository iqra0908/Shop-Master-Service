...
class EnDistanceBaseShippingOrderWidget
{
    ...
    {
        add_action(
            'woocommerce_order_actions', array($this, 'en_assign_order_details'), 11
        );
    }
    ...
    function en_assign_order_details($actions)
    {
        ...
        if (!empty($this->result_details) && count($this->result_details) > 0) {
            add_meta_box('en_additional_order_details', __('Additional Order Details', 'woocommerce'), array($this, 'en_add_meta_box_order_widget'), 'shop_order', 'side', 'low', 'core');
        }
