...
class EnDistanceBaseShippingRatesTemplateSettings
{
    ...
    public function getAllProfiles($shipping_classes = [])
    {
        ...
        if (isset($en_profiles) && !empty($en_profiles)) {
            foreach ($en_profiles as $key => $profile) {
                ...
                $profile->classes_details = (!empty($en_profiles_classes_arr)) ? $this->enGetSelectedClassesDetails($en_profiles_classes_arr) : [];
     ...
     */
    public function enGetSelectedClassesDetails($en_shipping_classes)
    {
