jQuery(document).ready(function ($) {
    ...
    var en_woo_addons_monthly_packg_dbsc = function (monthly_pckg) {
        var data = {selected_plan: monthly_pckg, action: 'en_woo_addons_upgrade_plan_submit_dbsc'};
        var params = "";
        ...
        if (jQuery('#en-dbsc-shipping-origin-form').validate().form()) {
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    en_loader_to_hide();
                    jQuery('#en_address_suggestion').remove();
                    ...
                    if (data.reload) {
                        ...
                        if (typeof en_obj.severity != 'undefined' && typeof en_obj.suggestedAddress != 'undefined' && typeof data.address_is_valid != 'undefined' && en_obj.severity == 'SUCCESS' && data.address_is_valid === false) {
                            jQuery('#en-dbsc-shipping-origin-form').prepend('<a href="javascript:void(0)" id="en_address_suggestion" >' + data.complete_Address + '</a>');
                            jQuery('html, body').animate({
                                'scrollTop': jQuery('#en_address_suggestion').position().top
                            });
    ...
    jQuery('#en-dbsc-add-rate').click(function (event) {
        ...
        jQuery.validator.addMethod("customValidationMaxQuote", function (value, element) {
            ...
            return true;
        }, "Min quote cannot be greater than equal to max quote.");
        ...
        jQuery.validator.addMethod("customValidationMaxCartValue", function (value, element) {
            ...
            return true;
        }, "Minimum cart value cannot be greater than or equal to maximum cart value.");
        ...
        jQuery(form_id).validate({
            ...
            messages: {
                ...
                en_dbsc_add_rate_max_quote: {
                    ...
                    customValidationMaxQuote: 'Min quote cannot be greater than equal to max quote.',
                    numbersOnly: 'Accept only numbers or numbers with maximum 2 decimal points e.g 15, 3.24.',
                },
                en_dbsc_add_rate_distance_adjustment: {
                    required: "Minimum shipping quote is required.",
                ...
                en_dbsc_add_rate_max_cart_value: {
                    customValidationMaxCartValue: 'Minimim cart value cannot be greater than or equal to maximum cart value.',
                    numbersOnly: 'Accept only numbers or numbers with maximum 2 decimal points e.g 15, 3.24.',
                },
                en_dbsc_add_rate_measured_by: {}
            }
        ...
        if (jQuery(form_id).validate().form()) {
            ...
            jQuery.ajax({
                type: 'POST',
                url: ajaxurl,
                data: {action: 'en_add_zone_rate', form_data: form_data},
                dataType: 'json',
                beforeSend: function () {
                    en_loader_to_show();
                },
        ...
        jQuery("#en-dbsc-shipping-profile-form").validate({
            rules: {
                ...
                en_shipping_classes: {
                    required: {
                        depends: function () {
                            return define_by_gp_condition() === 'en_for_specific_products';
                        }
                    }
                },
    ...
    jQuery('#en-add-shipping-zone').click(function (event) {
        ...
        if (jQuery('#en-dbsc-shipping-zone-form').validate().form()) {
            ...
            jQuery.ajax({
                type: 'POST',
                url: ajaxurl,
                data: {action: 'en_add_shipping_zone', form_data: form_data},
                dataType: 'json',
                beforeSend: function () {
                    en_loader_to_show();
                },
        ...
        success: function (data) {
            ...
            if (typeof data.response != 'undefined' && data.response == 'success') {
                if (profile_id == 1) {
                    ...
                    en_general_profile += 'Use the General Profile only for products that have the following Shipping Class(es)';
                    en_general_profile += '</div>';
                    en_general_profile += '</div>';
...
function en_dbsc_edit_shipping_zone(profile_id, origin_id, zone_id) {
    ...
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        data: {action: 'en_get_shipping_zone', zone_id: zone_id},
        dataType: 'json',
        beforeSend: function () {
            en_loader_to_show();
        },
        success: function (response) {
            ...
            if (response.message == 'success') {
                ...
                if (response.define_by === 'by_postal_code') {
                    ...
                    jQuery(form_id + ' #en_zone_postcodes').val(response.zip_codes);
                } else {
                    jQuery('.en-dbsc-postal-codes-div').hide();
...
function en_loader_to_show() {
    jQuery(".en_loader_overly_template").css({visibility: "visible", opacity: "1"});
}
