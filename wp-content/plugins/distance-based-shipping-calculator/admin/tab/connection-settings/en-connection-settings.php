...
class EnDistanceBaseShippingConnectionSettings
{
    ...
    static public function customer_subscription_status()
    {
        ...
        self::en_check_plugin_status($status);
     ...
     * Check plugin status.
     */
    static public function en_check_plugin_status($current_status)
    {
        $current_status = json_decode($current_status);
        $plugin_status = self::smart_street_api_curl_request("pluginType", '');
        ...
        if (
            ...
            if ($decoded_plugin_status->severity == "SUCCESS") {
                if ($decoded_plugin_status->pluginType == "trial") {
                    self::$append_with_current_status = '<span class="append_with_current_status">You are on a trial plan. Please log in to <a style="text-decoration:none" href="https://eniture.com">eniture.com</a> and click on the License tab. Next, click on the license&#39;s Edit button and enter your payment information. Afterward, make sure the <strong>Auto-renew</strong> field on this page reflects your choice of which plan to activate once the trial is over.</span>';
                }
            }
        }
        if ($decoded_plugin_status->severity == "SUCCESS") {
            if ($decoded_plugin_status->pluginType == "purchased") {
                self::$append_with_current_status = '<span class="append_with_current_status">You are currently on a trial plan but have provided a payment method for use after the trial is over. When the trial is over, your payment method will be used to activate the plan identified in the <strong>Auto-renew</strong> field on this page.</span>';
            }
        }
        if (
            isset($current_status->status->subscribedPackage->packageSCAC) &&
            ...
            if ($decoded_plugin_status->severity == "SUCCESS") {
                if ($decoded_plugin_status->pluginType == "purchased") {
                    self::$append_with_current_status = '<span class="append_with_current_status">When the current plan expires or is depleted, the option in the <strong>Auto-renew</strong> field will be used for the renewal.</span>';
                }
            }
        }
    }
        ...
        return
            [
                'en_connection_settings_license_key_distance_base_shipping' => [
                    ...
                    'name' => __('Plugin License Key ', 'woocommerce-settings-distance-base-shipping'),
                    'type' => 'text',
