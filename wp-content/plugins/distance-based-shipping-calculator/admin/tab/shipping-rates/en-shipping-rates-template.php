...
class EnDistanceBaseShippingQuoteSettings
{
    ...
    public function Load()
    {
        ...
        $this->wt_unit = get_option('woocommerce_weight_unit');
    ...
    public function getShippingClassesHtml($profile)
    {
        ...
        <div class="en_profile_classes" id="en-edit-profile-classes-<?php echo $this->profile_id; ?>">
            ...
            <?php
            if (isset($profile->classes_details) && !empty($profile->classes_details)) {
                foreach ($profile->classes_details as $key => $class) {
                    echo '<p>' . $class["name"] . '</p>';
                }
            } elseif (isset($profile->shipping_classes) && $profile->shipping_classes == '-1') {
                ?>
    ...
    public function getShippingRatesHtml($rate_details)
    {
        foreach ($rate_details as $key => $rate) {
            ...
            $calculate_for_show = ($rate->calculate_for == 'flat') ? '$'.$rate->rate : '$'.$rate->rate.' / '. $calculate_for;
            ?>
        ...
        <tr id="en_zone<?php echo $this->zone_id ?>_rate<?php echo $this->rate_id ?>">
            ...
            <td class="en-align-center"><?php echo number_format($rate->min_quote, 2, ".", "") . "-" . ($rate->max_quote > 0 ? $rate->max_quote : 'up') ?></td>
